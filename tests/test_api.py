...
class TestAPICase(unittest.TestCase):
    ...
    def test_store_raw(self):
        with open('test_resources/Openarms.sample.jsonl', 'r') as f:
            expected_data = [json.loads(line) for line in f.readlines() ]
    ...
    def test_preprocess_diffusion_metrics(self):
        ...
        cascade_ids = diffusion_metrics.get_cascade_ids(self.tmp_dataset)
        expected = {}
        for cascade_id in cascade_ids['tweet_id']:
            graph = diffusion_metrics.compute_propagation_tree(self.tmp_dataset, cascade_id)
            shortest_paths = diffusion_metrics.get_shortest_paths_to_original_tweet_over_time(graph)
            size_over_time = diffusion_metrics.compute_size_over_time(graph)
            depth_over_time = diffusion_metrics.compute_depth_over_time(shortest_paths)
            max_breadth_over_time = diffusion_metrics.compute_max_breadth_over_time(shortest_paths)
            structural_virality_over_time = diffusion_metrics.compute_structural_virality_over_time(graph)
            expected[cascade_id] = {'graph': graph, 'size_over_time': size_over_time,
        ...
        diffusion_metrics = DiffusionMetrics(host=preprocessor.host, port=preprocessor.port,
                                             reference_types=preprocessor.reference_types, egonet=egonet)

        actual = {}
        for cascade_id in cascade_ids['tweet_id']:
            graph = diffusion_metrics.get_propagation_tree(self.tmp_dataset, cascade_id)
            size_over_time = diffusion_metrics.get_size_over_time(self.tmp_dataset, cascade_id)
            depth_over_time = diffusion_metrics.get_depth_over_time(self.tmp_dataset, cascade_id)
            max_breadth_over_time = diffusion_metrics.get_max_breadth_over_time(self.tmp_dataset, cascade_id)
            structural_virality_over_time = diffusion_metrics.get_structural_virality_over_time(self.tmp_dataset,
                                                                                                cascade_id)
            actual[cascade_id] = {'graph': graph, 'size_over_time': size_over_time,
                                       'depth_over_time': depth_over_time,
                                       'max_breadth_over_time': max_breadth_over_time,
                                       'structural_virality_over_time': structural_virality_over_time}

        for cascade_id in cascade_ids['tweet_id']:
            expected_graph = expected[cascade_id]['graph']
            expected_size_over_time = expected[cascade_id]['size_over_time']
            expected_depth_over_time = expected[cascade_id]['depth_over_time']
            expected_max_breadth_over_time = expected[cascade_id]['max_breadth_over_time']
            expected_structural_virality_over_time = expected[cascade_id]['structural_virality_over_time']

            actual_graph = actual[cascade_id]['graph']
            actual_size_over_time = actual[cascade_id]['size_over_time']
            actual_depth_over_time = actual[cascade_id]['depth_over_time']
            actual_max_breadth_over_time = actual[cascade_id]['max_breadth_over_time']
            actual_structural_virality_over_time = actual[cascade_id]['structural_virality_over_time']
