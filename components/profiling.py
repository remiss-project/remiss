...
class ProfilingComponent(RemissComponent):
    ...
    def layout(self, params=None):
        return dbc.Collapse([
            dbc.Row([
                # First column: Radarplot and Vertical Barplot
                dbc.Col([
                            dcc.Loading(
                                id=f'loading-radarplot-{self.name}',
                                type='default',
                                children=self.radarplot_emotions.layout()  # Call radarplot layout
                            ),
                            
                            dcc.Loading(
                                id=f'loading-donut1-{self.name}',
                                type='default',
                                children=self.donut_plot_behaviour1.layout()  # Call donut plot 1 layout
                            ),
                            
                ], width=6),  # First column occupies half the width of the row

                # Second column: Donut Plots
                dbc.Col([
                            dcc.Loading(
                                id=f'loading-barplot-{self.name}',
                                type='default',
                                children=self.vertical_barplot_polarity.layout()  # Call vertical barplot layout
                            ),
                            dcc.Loading(
                                id=f'loading-donut2-{self.name}',
                                type='default',
                                children=self.donut_plot_behaviour2.layout()  # Call donut plot 2 layout
                            )
                ], width=6),  # Second column occupies half the width of the row
...
class RadarplotEmotionsComponent(BaseProfilingComponent):
    ...
    caption = 'Radarplot showing the distribution of emotions of the tweets produced by the specific user (in blue), compared to the average levels observed for Narrative Shapers, Fact Checkers, and Control Users'
...
class VerticalBarplotPolarityComponent(BaseProfilingComponent):
    ...
    caption = 'Polarity (Positive, Negative, and Neutral) and Hate Speech levels of tweets generated by the specific user (in blue), compared to the average levels observed for Narrative Shapers, Fact Checkers, and Control Users'
