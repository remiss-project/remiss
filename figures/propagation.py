...
class PropagationPlotFactoryOld(MongoPlotFactory):
    ...
    def prepopulate_egonet(self, force=False):
        if not self.cache_dir:
            logger.info('WARNING: Cache directory not set')
        ...
        for dataset in (pbar := tqdm(self.available_datasets, desc='Prepopulating egonet')):
            ...
            try:
                if force or not self.is_cached(dataset, 'hidden_network'):
                    ...
                    logger.info(f'Hidden network for {dataset} already cached')
            except Exception as ex:
                logger.info(f'Error prepopulating egonet for {dataset}: {ex}')
            ...
            try:
                if not force:
                    try:
                        ...
                        logger.info(f'{dataset} already prepopulated, skipping...')
                        continue
                ...
                logger.info(f'Error prepopulating propagation metrics for {dataset}: {e}')
    ...
    def get_egonet(self, dataset, user, depth):
        ...
        if user:
            try:
                ...
                logger.info(f'Computing neighbourhood for user {user} failed with error {ex}')
